...
/** GitHub Contents API（CDNなし・即時） */
async function fetchScheduleViaGithubContents() {
  const path = GITHUB_SCHEDULE_PATH;
  const res = await fetch(
    `https://api.github.com/repos/${GITHUB_REPO}/contents/${path}?t=${Date.now()}`,
    { cache: 'no-store', headers: githubApiHeaders() },
  );
  if (!res.ok) throw new Error(`GitHub Contents ${res.status}`);
  const data = await res.json();
  if (!data.content) throw new Error('GitHub Contents: empty');
...
/** Gist raw CDN を避ける（キャッシュでチームが古いままになるのを防ぐ） */
async function fetchGistScheduleViaApi() {
  const gistId = getGistId();
  if (!gistId) throw new Error('Gist未設定');
  ...
  const file = gist.files?.['schedule.json'];
  if (!file) throw new Error('Gistに schedule.json がありません');

  if (file.truncated || !file.content) {
    // truncated 時も Contents API を優先（raw CDNは最大5分遅れる）
    try {
      return await fetchScheduleViaGithubContents();
    } catch (_) {
      const rawUrl = file.raw_url || `https://gist.githubusercontent.com/${getGhUser()}/${gistId}/raw/schedule.json`;
      return fetchJsonFromUrl(rawUrl, 'GistRaw');
    }
  }
...
async function fetchSharedRaw() {
  ...
  // API優先（CDNキャッシュ回避）。Pages静的ファイルは最終フォールバック。
  const tasks = [];
  if (getGistId()) tasks.push(fetchGistScheduleViaApi());
  tasks.push(fetchScheduleViaGithubContents());
...
function updateWorkspaceNotice() {
  ...
  if (isLockedTeamViewer()) {
    ...
    const stale = isDataStale(S.syncMeta?.updatedAt, 1);
    ...
    el.innerHTML = stale
      ...
      : `👥 <strong>チーム閲覧</strong> — 最終更新: ${age} · 10秒ごとに自動更新 · <button type="button" class="ws-notice-dismiss" onclick="loadSharedData()">🔄 更新</button> <button type="button" class="ws-notice-dismiss" onclick="forceViewerRefresh()">♻️ 強制更新</button>`;
    return;
  }
...
async function pushToGithubRepo(payload, attempt = 0) {
  const token = getGithubWriteToken();
  ...
  const getRes = await fetch(`https://api.github.com/repos/${GITHUB_REPO}/contents/${path}?t=${Date.now()}`, {
    cache: 'no-store',
    headers,
  });
  let sha;
  ...
  if (putRes.status === 409 && attempt < 2) {
    return pushToGithubRepo(payload, attempt + 1);
  }
  if (!putRes.ok) {
...
async function pushScheduleToServer() {
  ...
  try {
    ...
    if (binId && isJsonBinPublic()) {
      ...
      // チーム閲覧は Gist を見る。Gist成功＝反映成功。Pagesは裏で再試行。
      await pushToGist(payload);
      pushToGithubRepo(payload).catch(err => {
        console.warn('Pages反映は後で再試行:', err);
        setTimeout(() => {
          pushToGithubRepo(payload).catch(e => console.warn('Pages再試行失敗:', e));
        }, 3000);
      });
    }
