...
let S = {
  ...
  viewMode: localStorage.getItem(VIEW_MODE_KEY) === '1',
  lockedViewer: false,
  syncMeta: null,
...
function updateWorkspaceNotice() {
  ...
  if (isLockedTeamViewer()) {
    ...
    el.innerHTML = stale
      ? `⚠️ <strong>データが古い可能性</strong>（最終更新: ${age}）— <button type="button" class="ws-notice-dismiss" onclick="loadSharedData()">🔄 今すぐ更新</button>`
      : `👥 <strong>チーム閲覧</strong> — データ更新: ${age} · 15秒ごとに自動更新 · <button type="button" class="ws-notice-dismiss" onclick="loadSharedData()">🔄 更新</button>`;
    return;
  }
  ...
  if (location.protocol === 'file:') {
    ...
    el.innerHTML = `📌 管理者URL · 編集は自動でチームに反映 · <button type="button" class="ws-notice-dismiss" onclick="dismissWorkspaceNotice()">閉じる</button>`;
  }
}
...
function applyViewModeUI() {
  ...
  if (banner) {
    ...
    if (locked) {
      banner.innerHTML = '👥 <strong>チーム閲覧</strong> — 編集・追加はできません。15秒ごとに自動更新されます。';
    } else {
...
function formatShootDeadline(entry) {
  ...
  return { text: fmtDate(deadline), overdue };
}
...
function wantsMobileTeamView() {
  if (!isMobile() || location.protocol === 'file:') return false;
  const p = new URLSearchParams(location.search);
  return p.get('view') !== '1' && p.get('edit') !== '1';
}
...
// ── Tab switching ─────────────────────────────────────────────────────────────
const VALID_TABS = new Set(['week', 'shoot', 'gantt', 'list', 'closed', 'subs']);
const TAB_ALIASES = { tasks: 'shoot' };
...
function switchTab(tab) {
  ...
  const hideFilters = tab === 'shoot' || tab === 'subs';
...
function render() {
  ...
  else if (S.tab === 'list') main.appendChild(mobile ? buildMobileCards() : buildList());
...
/** 三浦対応キュー：FB系のみ（前工程がすべて完了している案件） */
const TAK_WAITING_STAGES = ['FB', 'FB（施工・台本）'];
...
function buildWeekMobileCard(p, opts = {}) {
  ...
  return `<div class="${classes.join(' ')}">
    <div class="wk-card-top">
      ...
      ${typeBadge}
      <span class="cs" style="background:${cs.bg};color:${cs.fg}">${waitStatus}</span>
    </div>
    <div class="wk-card-title${done ? ' is-done' : ''}">${p.title}</div>
...
function buildShootingSchedule() {
  ...
  const shootDateCell = (ch, field, value) => {
    if (!editable) {
      return `<td>${value ? fmtDate(parseDate(value)) : '<span style="color:#bbb">—</span>'}</td>`;
    }
    return `<td><input type="date" class="shoot-date" data-ch="${escAttr(ch)}" data-field="${field}" value="${escAttr(value || '')}"></td>`;
  };
  ...
  if (editable) {
    const onFieldChange = (el) => {
      ...
      persist();
      if (field === 'lastPost' || field === 'postsPerWeek') {
...
// ── Bulk add ──────────────────────────────────────────────────────────────────
const BULK_SAME_FIELDS = ['ch', 'staff', 'type', 'stages', 'date', 'status'];
...
function wireBulkRowEvents(tr) {
  tr.querySelectorAll('.bulk-ch, .bulk-staff, .bulk-type, .bulk-status, .bulk-date, .bulk-date-offset').forEach(el => {
    el.addEventListener('change', refreshBulkDerivedDates);
    el.addEventListener('input', refreshBulkDerivedDates);
  });
}
...
function resolveBulkRows() {
  ...
  [...tbody.rows].forEach((tr, i) => {
    ...
    out.push({ tr, channel, staff, title, type, postDate, currentStatus: status, material, enabledStages });
  });
  return out;
}
...
function saveBulkProjects() {
  ...
  rows.forEach(({ channel, staff, title, type, postDate, currentStatus, material, enabledStages }) => {
    S.projects.push({
      id: uid(),
...
async function bootstrap() {
  ...
  if (wantsMobileTeamView()) {
    try {
      localStorage.removeItem(STORAGE_KEY);
      localStorage.removeItem(LOCAL_GUARD_KEY);
      localStorage.removeItem(SYNC_META_KEY);
    } catch (e) { /* ignore */ }
    const params = new URLSearchParams(location.search);
    params.set('view', '1');
    if (!params.has('tab')) params.set('tab', 'week');
